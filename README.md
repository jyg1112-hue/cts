...
- `railway.toml`: [Railway](https://railway.com) 배포용(Dockerfile 빌드) — 절차는 `docs/RAILWAY.md`
...
docker run -p 8000:8000 -e PORT=8000 port-ops-app
```

## 배포 (Railway)

요약 절차는 **`docs/RAILWAY.md`** 를 따르세요. (GitHub 연동 → Dockerfile 빌드 → `PORT` 로 기동 → `/api/health` 헬스체크)
