...
`backend/main.py`에서 스케줄/반출 저장 헬퍼 근처에 아래 함수 추가(과거 초안 — 현재는 로컬 JSON `data/*.json` 사용):
...
1. 배포된 공개 URL의 `/schedule` 에서 아이템 추가
2. 다른 기기/브라우저에서 같은 URL 열어 아이템이 보이는지 확인
3. 같은 호스트의 `/banchu` 동일하게 확인
4. 서버 `data/schedule_items.json`, `data/banchu_items.json` 파일이 갱신되는지 확인
