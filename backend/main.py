...
_ITEM_TABLE_LOCAL_FILES = {
    "schedule_items": BASE_DIR / "data" / "schedule_items.json",
    "banchu_items": BASE_DIR / "data" / "banchu_items.json",
}
_YARD_SIM_LOCAL_FILES = {
    "overall": BASE_DIR / "data" / "yard_sim_overall.json",
    "import": BASE_DIR / "data" / "yard_sim_import.json",
}
_ALLOWED_SIM_MODES = frozenset({"overall", "import"})


def _ensure_schedule_banchu_tables() -> None:
    """로컬 JSON 저장소 사용. No-op."""
...
def _db_fetch_items(table: str) -> list[tuple]:
    ...
    path = _ITEM_TABLE_LOCAL_FILES.get(table)
...
def _db_save_items(table: str, items: list[dict[str, Any]]) -> None:
    ...
    if table not in _ALLOWED_ITEM_TABLES:
        raise ValueError(f"허용되지 않은 테이블: {table}")
    if items and not any(item.get("id") for item in items):
        raise ValueError(
            "저장할 항목에 유효한 id가 없습니다. 항목마다 id가 있어야 서버에 저장됩니다."
        )
    path = _ITEM_TABLE_LOCAL_FILES.get(table)
    if not path:
        return
...
def _db_get_yard_sim(mode: str) -> dict | None:
    ...
    if mode not in _ALLOWED_SIM_MODES:
        return None
    path = _YARD_SIM_LOCAL_FILES.get(mode)
    if not path or not path.is_file():
        return None
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
...
def _uploaded_storage_files() -> list[dict[str, Any]]:
    """업로드 디렉터리에서 패턴에 맞는 엑셀 파일 목록 반환."""
    _ensure_upload_dir()
    out: list[dict[str, Any]] = []
    if not UNLOADING_UPLOAD_DIR.is_dir():
        return out
    try:
        for p in sorted(UNLOADING_UPLOAD_DIR.iterdir(), key=lambda x: x.name):
            if not p.is_file():
                continue
            name = p.name
            ...
            out.append({"name": name, "metadata": {"size": st.st_size}, "updated_at": updated_at})
    except OSError:
        return []
    return out
...
def _get_unloading_dataset() -> list[dict[str, Any]]:
    ...
    for f in sorted(storage_files, key=lambda x: x.get("name", "")):
        ...
        if not name:
            continue
        file_path = UNLOADING_UPLOAD_DIR / name
        if not file_path.is_file():
            continue
        for cargo, sheet in [("coal", UNLOADING_COAL_SHEET), ("nickel", UNLOADING_NICKEL_SHEET)]:
            try:
                all_rows.extend(_parse_unloading_sheet(cargo, sheet, file_path))
            except Exception:
                pass
    return all_rows
...
def delete_uploaded_unloading_excel(request: Request, file_name: str) -> JSONResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"파일 삭제 실패: {e}") from e
